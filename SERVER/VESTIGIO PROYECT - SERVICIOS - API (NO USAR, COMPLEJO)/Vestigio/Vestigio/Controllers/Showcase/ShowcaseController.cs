            ...
            string challengeSort = "level_asc")
        {
            ...
            if (User.Identity.IsAuthenticated)
            {
                var user = await _userManager.Users
                    ...
                    .Include(u => u.UnlockedProducts) // Incluir productos desbloqueados
                        .ThenInclude(up => up.Product) // Incluir detalles del producto
                    .Include(u => u.UnlockedProductLevels) // Incluir niveles desbloqueados
                ...
                if (user != null)
                {
                    ...
                    // Obtén la lista completa de productos desbloqueados
                    var unlockedProducts = await _context.Products
                       .Include(p => p.Images)
                       .Include(p => p.ProductCategories)
                       .Where(p => p.IsActive &&
                           (unlockedProductIds.Contains(p.Id) ||
                            unlockedProductLevels.Contains(p.RarityLevel)))
                       .ToListAsync();
                ...
                {
                    ViewBag.UnlockedProducts = new List<Product>();
                }
            }
